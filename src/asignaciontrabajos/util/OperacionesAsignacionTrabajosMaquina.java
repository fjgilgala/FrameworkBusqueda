...
public class OperacionesAsignacionTrabajosMaquina {
	...
	public static List<Integer> getCopiaSinAsignar(Integer tarea, List<Integer> sin_asignar) {
		List<Integer> clone = new ArrayList<Integer>(sin_asignar.size() - 1);
		for (Integer item : sin_asignar)
			if (tarea != item)
				clone.add(item);
		return clone;
	}
	...
	public static int[] copiaSti(int tarea, int valorSti, int[] sti) {
		int[] copia_sti = sti.clone();
		copia_sti[tarea] = valorSti; // st
		return copia_sti;
	}
	...
	public static int estimacionG(int sti, int tarea) {
		return Math.max(0, ValoresInstanciaAsignacionTrabajos.duracion[tarea] - (ValoresInstanciaAsignacionTrabajos.fechaVencimiento[tarea] - sti));
	}
